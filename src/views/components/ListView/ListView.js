...
import {getStreams} from '../../../core/init/actions/playingActions';
...
class ListItems extends Component {
    constructor(props){
        super(props);
        this.state = { player: false, subList: false, song: null }
    ...
    togglePlayer(song){
        this.setState({ player: !this.state.player, song })
    }
    toggleSubList(){
    ...
    renderSubList(){
        ...
        return tracks.map(track => {
                   return ( 
                       <ListItem key={track.url} button className={`${classes.nested} ${classes.listItem}`} onClick={() => this.handleButton(track, true)}>
                    <ListItemIcon>
...
export default connect(null, {getStreams})(ListView);